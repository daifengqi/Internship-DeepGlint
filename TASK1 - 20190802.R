...
# #1 金库
coffer_df <- as.data.frame(timehour)
for(i in seq_along(event_list)){
  ...
  if('SiteID' %in% colnames(ev)){
    ...
    colnames(processEV) <- c('timehour', ev_code)
...
# 合计2
coffer_sum_df <- cbind(coffer_df['timehour'],weigui_sum,yichang_sum)
...
# 1）发生事件统计
coffer_ga %>% 
  filter(时间 > as.POSIXct('2019-06-01') &
             时间 < as.POSIXct('2019-07-01')) %>%
...
coffer_sum <- 0.5*weigui_sum + 0.5*yichang_sum
# 危险度
coffer_sum_df <- cbind(coffer_df['timehour'],weigui_sum,yichang_sum)
colnames(coffer_sum_df) <- c('时间','违规进出','异常事件')
coffer_ga <- gather(coffer_sum_df, key = '类型', value = 'value', -时间)
...
jkd <- 10 - coffer_sum
coffer_jkd <- as.data.frame(cbind(coffer_df['timehour'], jkd))

coffer_jkd %>% 
  filter(timehour > as.POSIXct('2019-06-01') &
           timehour < as.POSIXct('2019-07-01')) %>%
  ggplot(aes(x=timehour, y = jkd)) + 
...
ggsave('金库安全度测量06.png',width=7.4,height=4)
...
coffer_jkd <- coffer_jkd %>% 
  filter(timehour > as.POSIXct('2019-06-01') &
           timehour < as.POSIXct('2019-07-01')) %>%
  mutate(state = if_else(jkd > 8,'高','中')) %>%
  mutate(state = if_else(jkd > 6,state,'低')) %>%
...
coffer_jkd <- coffer_jkd %>% 
  filter(timehour > as.POSIXct('2019-06-01') &
           timehour < as.POSIXct('2019-07-01')) %>%
  mutate(state2 = if_else(jkd > 8,3,2)) %>%
  mutate(state2 = if_else(jkd > 6,state2,1))
...
# #2 加钞间
bill_df <- as.data.frame(timehour)
for(i in seq_along(event_list)){
  ev <- read_xlsx(event_list[i])
  ev_file <- event_list[i]
  ev_code <- str_sub(ev_file, 11, 13)
...
# 合计2
bill_sum_df <- cbind(bill_df['timehour'],weiguiJC_sum,weiguiCZ_sum,yichang_sum)
...
jkd <- 10-bill_sum
bill_jkd <- as.data.frame(cbind(bill_df['timehour'], jkd))
...
bill_jkd <- bill_jkd %>% 
  filter(timehour > as.POSIXct('2019-06-01') &
           timehour < as.POSIXct('2019-07-01')) %>%
  mutate(state = if_else(jkd > 9.9999,'高','中')) %>%
  mutate(state = if_else(jkd > 9,state,'低')) %>%
  mutate(state = factor(state, levels = c('高','中','低')))
